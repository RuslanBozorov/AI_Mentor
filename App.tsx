...
const App: React.FC = () => {
  ...
  const loadTask = async (currentUser: UserProfile, isExam: boolean) => {
    ...
    try {
      ...
      console.error(e);
      if (e.message?.includes("API_KEY topilmadi") || e.message?.includes("API_KEY is not defined")) {
        setError("API_KEY_MISSING");
      } else if (e.message?.includes("QUOTA_EXCEEDED")) {
        setError("API limiti tugadi (429).");
      } else {
  ...
  // Fix: Added handleTaskSubmit to process student answer evaluation
  ...
  // API Kalit yo'qligi haqida maxsus ekran
  if (!apiKey) {
    return (
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6 text-center">
        <div className="max-w-md w-full bg-white rounded-[40px] p-10 shadow-2xl space-y-6 border-b-8 border-indigo-200">
          <div className="bg-indigo-100 w-24 h-24 rounded-full flex items-center justify-center mx-auto text-indigo-600 animate-bounce">
            <Key size={48} />
          </div>
          <h2 className="text-3xl font-black text-gray-900 leading-tight">Sozlash kerak!</h2>
          <div className="space-y-4 text-left">
            <p className="text-gray-600 font-medium">Loyiha Vercel-da ishlashi uchun <span className="font-black text-indigo-600">API_KEY</span> o'zgaruvchisini qo'shishingiz kerak.</p>
            <div className="bg-slate-50 p-4 rounded-2xl text-xs font-mono text-slate-500 border border-slate-100">
              Vercel Settings -> Environment Variables:<br/>
              Key: API_KEY<br/>
              Value: [Sizning_Kalitingiz]
            </div>
          </div>
          <p className="text-gray-400 text-sm">Kalitni qo'shgandan so'ng Deployments bo'limidan 'Redeploy' tugmasini bosing.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 font-sans antialiased text-gray-900 overflow-x-hidden">
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 p-4 shadow-sm">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button onClick={() => setActiveTab('profile')} className="hover:scale-110 active:scale-95 transition">
      ...
      <main className="max-w-4xl mx-auto p-4 md:p-8">
        {error && (
          <div className="mb-8 p-8 bg-white border-4 border-red-50 rounded-[40px] flex flex-col items-center text-center space-y-6 shadow-xl animate-in zoom-in duration-300">
            <div className="bg-red-100 p-5 rounded-full text-red-600">
               <AlertTriangle size={48} />
            </div>
            <div className="space-y-2">
               <h3 className="text-2xl font-black text-gray-900">Texnik to'xtalish</h3>
               <p className="text-gray-500 font-bold max-w-sm">{error === 'API_KEY_MISSING' ? "API_KEY o'zgaruvchisi Vercel-da topilmadi." : error}</p>
            </div>
            <button 
              onClick={() => { setError(null); if(currentNode) loadTask(user, currentNode.type === 'exam'); }}
              className="flex items-center space-x-2 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black hover:bg-indigo-700 transition active:scale-95 shadow-lg shadow-indigo-100"
            >
              <RefreshCcw size={20} />
        ...
        {isLoading && !lesson && !taskResponse && (
          <div className="fixed inset-0 z-50 bg-indigo-600 flex flex-col items-center justify-center text-white text-center p-8">
            ...
            <h2 className="text-4xl font-black mb-4 tracking-tighter italic">Mentor tayyorlamoqda...</h2>
            <p className="text-indigo-100 text-xl font-bold max-w-xs">Robotcha senga eng zo'r darsni qidiryapti!</p>
          </div>
        )}
        ...
        {activeTab === 'roadmap' && !lesson && (
          <div className="space-y-10 animate-in fade-in slide-in-from-top-4 duration-700">
            <div className="text-center space-y-2">
              ...
              <p className="text-indigo-600 font-black text-lg bg-indigo-50 inline-block px-6 py-2 rounded-full border border-indigo-100">{user.goal} Maqsadi</p>
            </div>
      ...
      {taskResponse && <FeedbackModal response={taskResponse} language={user.language} onNext={() => {
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[92%] max-w-md bg-white/95 backdrop-blur-2xl border border-white/20 p-4 z-40 shadow-[0_20px_50px_rgba(79,70,229,0.15)] rounded-[40px] flex justify-around">
          {[
